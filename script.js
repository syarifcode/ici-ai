// --- KONFIGURASI ---
const SYSTEM_PROMPT = "Kamu adalah Icila AI, asisten AI cerdas dan ceria. Pemilikmu adalah SyarifCode. Gaya bicaramu ramah, santai, seru, tetap profesional dan sangat membantu layaknya teman dekat dan memberikan penjelasan yang mudah dipahami. dan jangan gunakan simbol * dalam mengirim teks agar teks respon kamu terlihat rapi";
...
async function sendMessage() {
    ...
    try {
        ...
        renderMessageBubble('bot', `Ici (${currentMode}) ${loadingText}`, null, true);

        const data = await sendChatService(currentMode, userText, SYSTEM_PROMPT, currentSessionId, finalImageUrl);
        removeTemporaryMessages();
...
// --- UTILITIES (Bagian ini yang paling banyak berubah) ---
...
function renderMessageBubble(sender, text, imageUrl = null, isTemp = false) {
    ...
    let imgHTML = (imageUrl && imageUrl !== 'null') 
        ? `<img src="${imageUrl}" class="img-attachment" onerror="this.style.display='none'">` 
        : '';
    ...
    // FITUR BARU: Tambahkan Tombol Copy & Share (Khusus Bot)
    if (sender === 'bot' && !isTemp) {
        htmlContent += `
            <div class="msg-actions">
                <button class="action-btn" onclick="copyToClipboard(this)" title="Salin">
                    <i class="far fa-copy"></i> <span class="action-label">Salin</span>
                </button>
                <button class="action-btn" onclick="shareText(this)" title="Bagikan">
                    <i class="fas fa-share-alt"></i> <span class="action-label">Share</span>
                </button>
...
// Fungsi Share ke Sosmed/WA
window.shareText = function(btn) {
    const rawText = btn.parentElement.querySelector('.raw-text').innerText;
    if (navigator.share) {
        navigator.share({
            title: 'Jawaban Ici AI',
            text: rawText
        }).catch(err => console.log('Batal share'));
    } else {
        // Fallback kalau browser gak support share
        copyToClipboard(btn);
        alert("Browser tidak support Share langsung. Teks sudah disalin!");
    }
};
