...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderSidebar();
    loadCurrentSession();
    updateVisitorCount(); // JALANKAN PENGHITUNG DI SINI
...
async function downloadImage(url) {
    try {
        ...
        a.href = blobUrl;
        a.download = `Icila-Img-${Date.now()}.png`;
...
function renderMessageBubble(sender, text, imageUrl = null, isTemp = false) {
    ...
    let imgHTML = (imageUrl && imageUrl !== 'null') ? `<img src="${imageUrl}" class="img-attachment" onclick="window.open('${imageUrl}')">` : '';
...
window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); deferredPrompt = e; if(btnInstall) { btnInstall.classList.remove('hidden'); btnInstall.style.display = 'flex'; } });
if(btnInstall) { btnInstall.addEventListener('click', async () => { if (!deferredPrompt) return; deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; deferredPrompt = null; if (outcome === 'accepted') btnInstall.style.display = 'none'; }); }

// ==========================================
// --- FITUR VISITOR COUNTER (FIXED) ---
// ==========================================
async function updateVisitorCount() {
    const counterElement = document.getElementById('visitorCount');
    if (!counterElement) return;

    const FAKE_START = 13000; 
    // GANTI KE API YANG LEBIH STABIL (counterapi.dev)
    const NAMESPACE = 'icila_v3_syarif_fix'; 
    const KEY = 'visits';

    try {
        // Hit endpoint counterapi.dev
        const response = await fetch(`https://api.counterapi.dev/v1/${NAMESPACE}/${KEY}/up`);
        const data = await response.json();
        
        // Logikanya: 13.000 + Angka Real
        // Note: counterapi.dev mengembalikan { "count": 123 }
        const realCount = data.count || 0;
        const totalVisits = FAKE_START + realCount;

        animateValue(counterElement, FAKE_START, totalVisits, 2000);

    } catch (error) {
        console.log("Counter Error:", error);
        // Fallback jika API down
        counterElement.innerText = "13,000+";
    }
}
